...
@Client.on_callback_query(filters.regex('related_files') | filters.regex('starting_files') | filters.regex('findfiles'))
async def handle_callback_query(client, callback_query):
    ...
    if action == 'related_files':
        ...
        await callback_query.message.edit_text(related_files_message, reply_markup=None)

    elif action == 'starting_files':
        # Build the MongoDB query to search for files starting with the search query
        query = {
            'file_name': {"$regex": f"^{re.escape(search_query)}.*", "$options": "i"}
        }
        ...
        if results:
            starting_files_message = f'{len(results)} files found starting with "{search_query}" in the database:\n\n'
        else:
            starting_files_message = f'No files found starting with "{search_query}" in the database'

        await callback_query.message.edit_text(starting_files_message, reply_markup=None)

    elif action == 'findfiles':
        await callback_query.message.edit_reply_markup(reply_markup=None)

    buttons = [
        [
            InlineKeyboardButton("🌟 Find Related Name Files", callback_data=f"related_files:{page+1}:{search_query}")
        ],
        [
